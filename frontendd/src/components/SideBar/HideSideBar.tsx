...
const SideBarToggle = ({ isSidebar, setIsSidebar }: props) => {
  return (
    <div>
      <button
        onClick={() => setIsSidebar(!isSidebar)}
        className={'fixed bottom-12 cursor-pointer flex space-x-3 items-center mt-4 pl-6 transition text-mediumGrey'}
      >
      ...
      <article className='absolute bottom-1 left-5 text-center text-[10px] text-mediumGrey' style={{ fontStyle: 'italic' }}>
        <a href="https://frontendmentor.io" target={'_blank'}>
          <div className='flex items-center justify-center'>
            ...
            <img className='rounded-full h-[20px] w-[20px]' src="/assets/fe_icon.png" />
          </div>
        </a>
      </article>
    </div>
